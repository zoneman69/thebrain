import time
from smbus2 import SMBus

BUS=7
ADDR=0x40

MODE1=0x00
MODE2=0x01
PRESCALE=0xFE
LED0_ON_L=0x06

def r(bus, reg): return bus.read_byte_data(ADDR, reg)
def w(bus, reg, val): bus.write_byte_data(ADDR, reg, val & 0xFF)

def set_pwm(bus, ch, on, off):
    reg = LED0_ON_L + 4*ch
    w(bus, reg+0, on & 0xFF)
    w(bus, reg+1, (on >> 8) & 0xFF)
    w(bus, reg+2, off & 0xFF)
    w(bus, reg+3, (off >> 8) & 0x0F)

def set_freq(bus, hz):
    osc = 25_000_000.0
    prescaleval = (osc / (4096.0 * hz)) - 1.0
    prescale = int(prescaleval + 0.5)

    old = r(bus, MODE1)
    w(bus, MODE1, (old & 0x7F) | 0x10)   # sleep
    w(bus, PRESCALE, prescale)
    w(bus, MODE1, (old & 0xEF))          # wake (clear sleep)
    time.sleep(0.005)
    w(bus, MODE1, (old & 0xEF) | 0x80)   # restart
    time.sleep(0.005)

with SMBus(BUS) as bus:
    # Hard init
    w(bus, MODE1, 0x00)          # normal, no sleep
    time.sleep(0.01)
    w(bus, MODE2, 0x04)          # OUTDRV (totem pole) good for servos
    time.sleep(0.01)

    set_freq(bus, 50)

    # Clear all channels
    for ch in range(16):
        set_pwm(bus, ch, 0, 0)

    # Now write CH0 OFF to a few values and read back
    for off in (205, 307, 410):
        set_pwm(bus, 0, 0, off)
        back = [r(bus, LED0_ON_L+i) for i in range(4)]
        print("OFF", off, "CH0 regs:", [hex(x) for x in back])
        time.sleep(1)

